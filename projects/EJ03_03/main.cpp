...
void render(uint32_t VAO, const Shader& shader) {
    ...
    //se utiliza el seno para que el rango esté entre 0 y 1 y se divide para que el triangulo no salga de la pantalla.
    float position = sin(glfwGetTime()) / 2.0f;

    shader.use();
    shader.set("addColor", 0.2f, 0.0f, 0.0f);
    shader.set("myPosition", position, position, 0.0f);
